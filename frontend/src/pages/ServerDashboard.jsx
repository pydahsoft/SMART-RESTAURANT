...
const ServerDashboard = () => {
  // Auth hooks
  const { token } = useAuth();
  const { isWaiterMode, waiterInfo } = useWaiterAuth();
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      setOrders([]);
      if (!isWaiterMode || err.message !== 'Authentication required') {
  ...
  const fetchCompletedOrders = useCallback(async () => {
    // Skip if in waiter mode or no token available
    if (isWaiterMode || !token) {
      setCompletedOrders([]);
      return;
    }
    ...
    try {
      ...
      setCompletedOrders([]);
      // Don't set the main error state for completed orders
    }
  }, [token, isWaiterMode]);
  ...
  useEffect(() => {
    ...
    let interval;
    if ((isWaiterMode && localStorage.getItem('waiterToken')) || (!isWaiterMode && token)) {
  ...
  }, [fetchOrders, fetchCompletedOrders, isWaiterMode, token]);
      ...
      >
        ...
        <DialogContent>
          {qrError ? (
            ...
            <>
              {qrCodeString && !showPaymentConfirmation && (
                <Box sx={{ 
                  display: 'flex',
                  flexDirection: 'column',
                  alignItems: 'center',
              ...
              {showPaymentConfirmation && (
                <>
                  <DialogContentText>
                    Please confirm that you have received the payment of ₹{upiDialog.amount.toFixed(2)}
                  </DialogContentText>
